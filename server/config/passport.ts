...
passport.deserializeUser<number>(async (id, done) => {
  try {
    const user = await db.prepare('SELECT * FROM users WHERE id = ?').get(id) as User | null;
    done(null, user);
  } catch (error) {
    done(error, null);
  }
      ...
      new PatreonStrategy(
        ...
        async (accessToken: string, refreshToken: string, profile: Profile, done: (error: any, user?: any) => void) => {
          try {
            ...
            let user = await db.prepare('SELECT * FROM users WHERE patreon_id = ?').get(patreonId) as User | null;
            ...
            if (user) {
              ...
              await db.prepare(`
                ...
                WHERE patreon_id = ?
              `).run(username, email, avatarUrl, safeAccessToken, safeRefreshToken, patreonId);

              user = await db.prepare('SELECT * FROM users WHERE patreon_id = ?').get(patreonId) as User;
            } else {
              // Create new user
              const result = await db.prepare(`
                INSERT INTO users (patreon_id, username, email, avatar_url, access_token, refresh_token, role)
                VALUES (?, ?, ?, ?, ?, ?, 'user')
              `).run(patreonId, username, email, avatarUrl, safeAccessToken, safeRefreshToken);
